...
class MenuOptimizationApp {
  ...
  async loadMenus() {
    try {
      ...
      // dateLabel を API に渡す（サーバーが内部で YYYY-MM-DD に変換）
      const response = await fetch(`/menus?dateLabel=${encodeURIComponent(selectedDateLabel)}`);
      if (!response.ok) {
  ...
  async loadAvailableDates() {
    try {
      ...
      dates.forEach((date, index) => {
        ...
        option.value = date; // YYYY-MM-DD 形式（内部的に変換）
        option.textContent = date; // "1/12(月)" 形式で表示
  ...
  async runOptimization() {
    ...
    try {
      ...
      const isoDate = this.dateLabelToISOString(dateLabelValue);
      if (!isoDate) {
