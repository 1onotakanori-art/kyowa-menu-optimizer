...
### STEP 1: 固定のみ合計を「常に見える」表示にする
- メニュー一覧をスクロールしても、固定のみ合計（E/P/F/C/V・差分）が確認できる
- 上部固定 or 下部固定のどちらが自然かを実機（iPhone）で確認

### STEP 2: 状態操作の最終UIを確定する
- メニュー枠（行）タップ: 推奨↔除外
- 固定スイッチ（iOSトグル風）: 固定 ON/OFF
  - 固定 ON で除外解除
  - 固定 ON のときに行タップで「固定解除→除外」
- 既存の凡例（推奨/固定/除外）表示と整合させる

### STEP 3: 最適化との接続（混同防止）
- 最適化実行時に固定の扱いを明示（固定を制約として使う/使わない）
- 手動（固定のみ合計）と最適化結果の表現を揃える
