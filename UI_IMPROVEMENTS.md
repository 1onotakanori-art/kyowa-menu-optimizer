...
### STEP 1: 状態モデルを明確化（内部仕様）
- 各メニューに持たせる状態を整理
  - `fixed: boolean`（手動集計に入る）
  - `excluded: boolean`（最適化候補から除外）
  - ルール: `excluded === true` のとき `fixed === true` は不可（固定を優先するか、固定操作時に除外解除）
- 既存の「推奨/固定/除外の循環タップ」仕様をどう移行するか決める（互換 or 破壊的変更）

### STEP 2: 操作UIを再設計（表示と入力）
- メニュー行の操作を分離（例）
  - **固定**: ピン/固定ボタン（タップでON/OFF）
  - **除外**: 除外トグル（タップでON/OFF）
  - **推奨**: fixed/excluded が両方OFFのデフォルト見た目
- 誤操作を減らすため、操作部位（ボタン領域）を明確化し、行タップ自体の意味（詳細表示/何もしない等）を統一

### STEP 3: 固定のみ集計（計算ロジック）
- 「固定メニュー一覧」から合計を算出
  - 合計: E/P/F/C/V（+ 価格が取れていれば価格合計）
  - 目標値が選択されている項目は差分（±）も算出
- 集計結果は、メニューを固定/解除した瞬間に更新（ライブ更新）

### STEP 4: メニュータブにサマリー表示を追加
- 「固定のみ集計サマリー」カードをメニュータブ上部に表示
  - 固定件数、各栄養合計、目標との差分
  - 「固定を全解除」など最低限の操作（任意）
- iPhoneでも見やすい密度にする（折り返し/2段表示など）

### STEP 5: 最適化との接続を整理（混同防止）
- 最適化実行時に固定の扱いを明示
  - 例: 「固定を必ず含めて最適化」 / 「固定を無視して最適化（手動集計のみ）」
- 最適化結果タブの表示（固定/提案）と、手動集計サマリーの表現を統一

### STEP 6: 既存データ/状態の移行とリグレッション確認
- 既存の状態（推奨/固定/除外）が保存されている場合、互換処理を用意
- 確認項目
  - 固定のみ集計が常に正しい
  - 固定と除外が矛盾しない
  - iPhone/PCで操作しやすい（タップ領域・スクロール）
