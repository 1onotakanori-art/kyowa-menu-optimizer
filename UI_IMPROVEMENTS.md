...
### 1. メニュー10個制限の原因（ステータス：要スクレイプ再実行）
- **調査結果**:
  - 2026/01/13は41個のメニューが正常に取得されている
  - 2026/01/14以降は10個しか取得されていない
  - JavaScript側には10個制限がないため、スクレイパーまたはサイト側の問題
- **推定原因**:
  - Kyowaメニューサイトで1/14以降のメニューが10個しかない
  - スクレイパーの展開ボタン処理が正しく動作していない
  - サイトのDOM構造が日付により異なる
- **解決策**: スクレイプを再実行して確認
  ```bash
  node prescrap.js
  ```
- **対象ファイル**: `src/scraper/fetchMenus.js`, `prescrap.js`
