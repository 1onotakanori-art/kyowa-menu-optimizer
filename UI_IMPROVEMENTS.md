...
### 1. 設定タブでメニュー10種のみ表示される問題（ステータス：確認待ち）
- **報告内容**: 「メニューが10種のみ表示される」
- **調査結果**:
  - 最適化実行時の選択上限は既に `availableMenus.length` に修正済み（全メニュー対象）
  - UI 表示時の max-height は `500px` に設定（スクロール可能）
  - JavaScript での制限は確認されず
- **質問**: 
  - 「10種のみ」とは、メニュー選択画面での読み込み数ですか？
  - それともスクロールしても10種しか見えないですか？
  - 具体的な状況（どの画面で、スクロールは可能かなど）をご教示ください

### 2. iPhone での実装確認
- **確認項目**:
  - ✅ 栄養成分色（Option B: 明色背景＋黒文字）の視認性
  - ✅ 小数点表示（E, P, F, C, V の数値に小数点が正しく表示されているか）
  - ⏳ メニュータップ時の3状態サイクル（推奨→固定→除外→推奨）
  - ⏳ メニュー栄養値の等間隔表示
  - ⏳ 凡例の3セクションレイアウト
  - ⏳ 結果画面のメニュー表示（E, P, F, C, V 順）
  - ⏳ 除外ボタンの位置（メニュー名の右）
