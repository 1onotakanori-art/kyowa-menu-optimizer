...
## Phase 3: API化とバックエンド統合 🚀

### 目標
機械学習モデルをREST APIとして公開し、既存のExpressサーバーに統合

### タスク

#### 3-1. Python推薦APIサーバー構築
- [ ] FastAPIまたはFlaskで軽量APIサーバー作成
- [ ] エンドポイント設計:
  - `POST /api/ml/recommend` - メニューリストを受け取り、推薦スコアを返す
  - `POST /api/ml/predict-batch` - 日付を指定してその日の全メニューを推薦
  - `GET /api/ml/model-info` - モデル情報取得
- [ ] CORSとセキュリティ設定
- [ ] エラーハンドリング

#### 3-2. Node.js側からPython APIを呼び出し
- [ ] `src/ml/` ディレクトリ作成
- [ ] Python APIクライアント実装
- [ ] 既存の `src/optimizer/optimizeMenus.js` に推薦スコアを統合
...
#### 4-1. UIデザイン
- [ ] 推薦メニューのビジュアル表示（⭐マークなど）
- [ ] 推薦スコアの表示方法決定
- [ ] 「AI推薦順に並び替え」ボタン追加

#### 4-2. フロントエンド実装
- [ ] `app.js` に推薦API呼び出し機能追加
- [ ] メニューカードに推薦スコア表示
- [ ] ソート機能追加（通常順 / AI推薦順）
- [ ] ローディングインジケーター
